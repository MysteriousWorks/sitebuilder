...
type BuilderProps = {
  initialPageMetaData: PageMetaData;
  initialPageContent?: Root;
} & React.PropsWithChildren;

const BuilderRoot: React.FunctionComponent<BuilderProps> = ({
  initialPageMetaData,
  initialPageContent,
  children,
}) => {
  let pageId =
    initialPageMetaData?.id ||
    createHash("md5")
      .update(initialPageMetaData.path)
      .digest("base64")
      .toString();
...
export const Builder: React.FunctionComponent<BuilderProps> = ({
  initialPageMetaData,
  initialPageContent,
  children,
}) => {
